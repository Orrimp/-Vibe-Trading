...
1. **Given** the repo history at `simple-strategy-bear-survey`'s landing commits (`git log -- spec/v1/simple-strategy-bear-survey`), **when** the recorded verification for `simple-strategy-bear-survey` is replayed (tests, reports under `spec/v1/simple-strategy-bear-survey/reports/` where present, render proofs where UI-facing), **then** the shipped behaviour holds: the two-stage bear-market survey over the 2021-22 corpus firming ship-passive on the deepest bear evidence.
...
- Provenance: `git log -- spec/v1/simple-strategy-bear-survey` (full narrative); reports under `spec/v1/simple-strategy-bear-survey/reports/` where present (anchored report bodies are byte-immutable - ADR-0038 §D6).
