...
1. **Given** the repo history at `v15a-mean-reversion-pairs`'s landing commits (`git log -- spec/v1/v15a-mean-reversion-pairs`), **when** the recorded verification for `v15a-mean-reversion-pairs` is replayed (tests, reports under `evidence/v1/v15a-mean-reversion-pairs/reports/` where present, render proofs where UI-facing), **then** the shipped behaviour holds: mean-reversion on z-scored pairs, landing the pairs/portfolio plumbing.
...
- Provenance: `git log -- spec/v1/v15a-mean-reversion-pairs` (full narrative); reports under `evidence/v1/v15a-mean-reversion-pairs/reports/` where present (anchored report bodies are byte-immutable - ADR-0038 §D6).
