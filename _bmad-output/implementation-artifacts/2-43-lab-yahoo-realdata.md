...
1. **Given** the repo history at `lab-yahoo-realdata`'s landing commits (`git log -- spec/v1/lab-yahoo-realdata`), **when** the recorded verification for `lab-yahoo-realdata` is replayed (tests, reports under `spec/v1/lab-yahoo-realdata/reports/` where present, render proofs where UI-facing), **then** the shipped behaviour holds: the multi-asset Yahoo data pivot for the Lab.
...
- Provenance: `git log -- spec/v1/lab-yahoo-realdata` (full narrative); reports under `spec/v1/lab-yahoo-realdata/reports/` where present (anchored report bodies are byte-immutable - ADR-0038 §D6).
