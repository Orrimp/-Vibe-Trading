...
1. **Given** the repo history at `advisor-llm-narration`'s landing commits (`git log -- spec/v1/advisor-llm-narration`), **when** the recorded verification for `advisor-llm-narration` is replayed (tests, reports under `spec/v1/advisor-llm-narration/reports/` where present, render proofs where UI-facing), **then** the shipped behaviour holds: opt-in faithful LLM narration of the crowned pick (F9) guarded by the deterministic check_faithful post-check with templated-copy fallback, plus the F6+F9 live last-mile display recipes.
...
- Provenance: `git log -- spec/v1/advisor-llm-narration` (full narrative); reports under `spec/v1/advisor-llm-narration/reports/` where present (anchored report bodies are byte-immutable - ADR-0038 §D6).
