...
1. **Given** the repo history at `v1-5b-multi-venue`'s landing commits (`git log -- spec/v1/v1-5b-multi-venue`), **when** the recorded verification for `v1-5b-multi-venue` is replayed (tests, reports under `evidence/v1/v1-5b-multi-venue/reports/` where present, render proofs where UI-facing), **then** the shipped behaviour holds: multi-venue support plus 1-second aggregated trades.
...
- Provenance: `git log -- spec/v1/v1-5b-multi-venue` (full narrative); reports under `evidence/v1/v1-5b-multi-venue/reports/` where present (anchored report bodies are byte-immutable - ADR-0038 §D6).
