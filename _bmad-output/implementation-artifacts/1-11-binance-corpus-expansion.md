...
1. **Given** the repo history at `binance-corpus-expansion`'s landing commits (`git log -- spec/v1/binance-corpus-expansion`), **when** the recorded verification for `binance-corpus-expansion` is replayed (tests, reports under `evidence/v1/binance-corpus-expansion/reports/` where present, render proofs where UI-facing), **then** the shipped behaviour holds: the wider down-market Binance corpus (data/binance/, ADR-0032 pins) underpinning the bear surveys.
...
- Provenance: `git log -- spec/v1/binance-corpus-expansion` (full narrative); reports under `evidence/v1/binance-corpus-expansion/reports/` where present (anchored report bodies are byte-immutable - ADR-0038 §D6).
