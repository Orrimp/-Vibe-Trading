...
1. **Given** the repo history at `v05-composed-strategies`'s landing commits (`git log -- spec/v1/v05-composed-strategies`), **when** the recorded verification for `v05-composed-strategies` is replayed (tests, reports under `spec/v1/v05-composed-strategies/reports/` where present, render proofs where UI-facing), **then** the shipped behaviour holds: hot-loadable TOML indicator/rule strategy assemblies (MACD + RSI + Bollinger) with atomic swap on file change.
...
- Provenance: `git log -- spec/v1/v05-composed-strategies` (full narrative); reports under `spec/v1/v05-composed-strategies/reports/` where present (anchored report bodies are byte-immutable - ADR-0038 §D6).
