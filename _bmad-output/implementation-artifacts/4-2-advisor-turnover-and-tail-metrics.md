...
1. **Given** the repo history at `advisor-turnover-and-tail-metrics`'s landing commits (`git log -- spec/v2/advisor-turnover-and-tail-metrics`), **when** the recorded verification for `advisor-turnover-and-tail-metrics` is replayed (tests, reports under `spec/v2/advisor-turnover-and-tail-metrics/reports/` where present, render proofs where UI-facing), **then** the shipped behaviour holds: the Churn (turnover) column + the coherent "Risk story" tail block (CVaR / median / skew) on the leaderboard (P1-1 + P1-2).
...
- Provenance: `git log -- spec/v2/advisor-turnover-and-tail-metrics` (full narrative); reports under `spec/v2/advisor-turnover-and-tail-metrics/reports/` where present (anchored report bodies are byte-immutable - ADR-0038 §D6).
