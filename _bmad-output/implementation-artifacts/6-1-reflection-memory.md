...
1. **Given** the repo history at `reflection-memory`'s landing commits (`git log -- spec/v1/reflection-memory`), **when** the recorded verification for `reflection-memory` is replayed (tests, reports under `evidence/v1/reflection-memory/reports/` where present, render proofs where UI-facing), **then** the shipped behaviour holds: the persistent lesson-card store with retrieval at decision time, wired through the sanctioned ADR-0041 layering seam (+ trader wiring).
...
- Provenance: `git log -- spec/v1/reflection-memory` (full narrative); reports under `evidence/v1/reflection-memory/reports/` where present (anchored report bodies are byte-immutable - ADR-0038 §D6).
