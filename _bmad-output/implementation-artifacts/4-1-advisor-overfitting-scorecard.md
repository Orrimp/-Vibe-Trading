...
1. **Given** the repo history at `advisor-overfitting-scorecard`'s landing commits (`git log -- spec/v2/advisor-overfitting-scorecard`), **when** the recorded verification for `advisor-overfitting-scorecard` is replayed (tests, reports under `evidence/v2/advisor-overfitting-scorecard/reports/` where present, render proofs where UI-facing), **then** the shipped behaviour holds: the report-only overfitting scorecard (N_eff -> DSR -> MinBTL) beside every recommendation - additive to the FROZEN gate, never a veto (P0-1).
...
- Provenance: `git log -- spec/v2/advisor-overfitting-scorecard` (full narrative); reports under `evidence/v2/advisor-overfitting-scorecard/reports/` where present (anchored report bodies are byte-immutable - ADR-0038 §D6).
