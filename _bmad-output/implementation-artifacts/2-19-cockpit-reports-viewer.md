...
1. **Given** the repo history at `cockpit-reports-viewer`'s landing commits (`git log -- spec/v1/cockpit-reports-viewer`), **when** the recorded verification for `cockpit-reports-viewer` is replayed (tests, reports under `spec/v1/cockpit-reports-viewer/reports/` where present, render proofs where UI-facing), **then** the shipped behaviour holds: the in-cockpit Reports screen browsing the committed backtest-report corpus with KPI strip + markdown body + equity curve.
...
- Provenance: `git log -- spec/v1/cockpit-reports-viewer` (full narrative); reports under `spec/v1/cockpit-reports-viewer/reports/` where present (anchored report bodies are byte-immutable - ADR-0038 §D6).
