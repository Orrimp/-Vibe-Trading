...
1. **Given** the repo history at `advisor-forward-plan`'s landing commits (`git log -- spec/v1/advisor-forward-plan`), **when** the recorded verification for `advisor-forward-plan` is replayed (tests, reports under `evidence/v1/advisor-forward-plan/reports/` where present, render proofs where UI-facing), **then** the shipped behaviour holds: the honest, conditional forward buy/sell plan (F6): IF/THEN rules faithful to the real TOMLs, not-a-prediction framing, configurable 1-30d horizon.
...
- Provenance: `git log -- spec/v1/advisor-forward-plan` (full narrative); reports under `evidence/v1/advisor-forward-plan/reports/` where present (anchored report bodies are byte-immutable - ADR-0038 §D6).
