...
| **T512** — `agent::EventBus` three new broadcast channels + publishers | `bus.strategy_loaded_rx() / strategy_swapped_rx() / strategy_error_rx()` + `publish_strategy_loaded(..)` etc. + a v0.5 extension section appended to `spec/reports/dev-week2-broadcast-api-2026-04-18.md`. | Without these the `ui::live` subscribers in T526 cannot compile. |
