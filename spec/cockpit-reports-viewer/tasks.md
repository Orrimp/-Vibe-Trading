...
- [ ] Create `crates/ui/src/reports/mod.rs` (mirror `baseline/mod.rs`):
      `pub mod loader; pub mod state; pub mod body_render;` + `pub use
      state::{ReportsScreenState, ReportEntry};`.
- [ ] Register the module in `lib.rs`: add `pub mod reports;` next to
      `pub mod baseline;` (`lib.rs:52`), with a doc-comment mirroring the
      baseline one.
- [ ] Create `crates/ui/src/reports/loader.rs`. **Move** verbatim from
...
- [ ] Move the `parse_front_matter_extracts_scenario` test
...
- [ ] `cargo build -p ui` + `cargo test -p ui --bin viewer` green; the
      `viewer_read_only.rs` build-time grep stays green (no write path added).

### M-DEV-2 — All-slug discovery scan (R1 / R7 / AC1 / AC3)

- [ ] In `reports/loader.rs`, add `pub fn discover_reports() -> Vec<ReportEntry>`:
...
- [ ] Sort the result deterministically by `(slug, file_stem)` (stable list
      order + reproducible snapshots).
- [ ] K2 never-panic: any unreadable dir → skip with a `tracing::debug!`
      breadcrumb, never panic (mirror `registry_read.rs:92` +
      `baseline/loader.rs:77`). Absent `spec/` → empty `Vec`.
- [ ] Confirm the `robustness-sweep-*.md` + `test-*.md` families are
      excluded by the `backtest-` filter (not silently dropped — they never
      match).

### M-DEV-3 — `ReportsScreenState` + `ReportEntry` (D1)

- [ ] Create `crates/ui/src/reports/state.rs` (mirror `baseline/state.rs`):
      ```rust
...
- [ ] `impl Default`: `discovered: Loading`, `selected: None`, `loaded: Loading`.
- [ ] Add `pub fn load_selection(&mut self, idx: usize)`: look up the
...
- [ ] Add `pub fn load_into(model: &mut crate::state::Cockpit)`: run the
...
- [ ] `Message` enum (`state.rs:~1951`, near `BaselineSelectYear`): add
      `ReportsSelect(usize)` — typed index, NEVER a String/PathBuf payload
      (R1). Doc-comment noting the PathBuf lives in state.
- [ ] `update` (`state.rs:~2824`, near the `BaselineSelectYear` arm): add
      ```rust
...
- [ ] `sidebar_nav::label_for` (`sidebar_nav.rs:35`): add
      `Screen::Reports => REPORTS_SIDEBAR_LABEL` arm.
- [ ] Confirm the two sidebar edits put `Reports` in the SAME relative
      position (Models < Reports < Trail) — the flatten-invariant test is the
      guard (M-TEST-3).

### M-DEV-6 — Strings: `REPORTS_*` block (R5 / AC6)

- [ ] `strings.rs` (after the BASELINE block, `:1834`): add the five
...
- [ ] **Right detail:** `selected == None` → `REPORTS_SELECT_PROMPT`.
...
- [ ] Confirm: curve/band render their built-in Empty body for
...
- [ ] `cockpit.rs` (next to `ui::baseline::load_into(&mut cockpit)` at
      `:238`): add `ui::reports::state::load_into(&mut cockpit);`.
- [ ] `cockpit_live.rs` (next to the baseline load_into at `:618`): add
      `ui::reports::state::load_into(&mut cockpit);`.
- [ ] Confirm the default screen stays `Screen::Live` in both bins (D5 — do
...
- [ ] `load_report` on a valid-`## Summary` fixture → metrics `Ready`; on a
      `## Summary`-less fixture → metrics `Error(NoSummaryHeading)`, no panic;
      companion-less report → equity `Empty`.
- [ ] `parse_front_matter_extracts_scenario` (moved from the bin) still passes.

### M-TEST-2 — State unit tests (`reports/state.rs` `#[cfg(test)]`) (AC3)

- [ ] `Default` = (`discovered: Loading`, `selected: None`, `loaded: Loading`).
- [ ] `load_selection(idx)` sets `selected = Some(idx)` + transitions
      `loaded`; a vanished-path index → `loaded: Error`, no panic.

### M-TEST-3 — Sidebar flatten-invariant (`theme.rs:1607`) (AC6)

- [ ] `sidebar_groups_phase_c__flatten_matches_phase_a` passes (auto-green
      once both consts get `Reports` in the same relative position).
- [ ] Add a one-line assertion that `Reports` sits between `Models` and
      `Trail` in the flattened list.

### M-TEST-4 — Panel snapshot, both themes (`tests/panel_snapshots.rs`) (AC2/AC6)

- [ ] New `reports_screen` mod (mirror `baseline_screen` at `:3248`):
...
- [ ] Use a deterministic fixture `ReportsScreenState` (hand-built
      `ReportEntry` list + a `Ready` `ReportLoadResult`) so the snapshot is
      checkout-independent.
- [ ] Assert all copy resolves through `strings::REPORTS_*` (no hardcoded
...
- [ ] `headless_emulator_paints_reports_route` (mirror
...
- [ ] Empty-list degrade verified for a fixtures-only checkout (the scan
      returns `Empty`/empty `Vec`, still a clean paint).

### M-TEST-6 — Lumen + crate-edge review (AC6/AC7)

- [ ] `tests/consistency.rs` / `tests/contrast.rs` /
      `tests/layout_invariants.rs` stay green.
- [ ] Review: loader is pure-`ui` over `core` + `reports` + `std::fs` (both
      already deps); three render widgets + `body_render` reused verbatim;
      **zero new theme tokens, zero new widgets, no new crate edge**.
- [ ] `cargo clippy -p ui -- -D warnings` introduces no new warnings (the
      pre-existing ~140 pedantic lints are untouched).

### M-TEST-7 — Regression floor (read-only feature) (AC7)

- [ ] `scripts/verify_anchors.sh` green — Reports only READS committed
      reports; no anchored `spec/*/reports/` file is touched, no new backtest
      anchor.
- [ ] Full `cargo test -p ui` green (the existing snapshot suite + new tests).
- [ ] HANDOFF → tester emits the `## Verification` link + the test report.
