...
- [ ] **T-OG.5** — Create `crates/backtest/tests/realdata_simple_strategy_overfit_guard.rs` (option a).
      **Copy `load_year_bars` verbatim** from
      ...
      Add `run_one_path(sym, strat, path_bars) -> Option<stats::PathMetrics>`: call `run_scenario` with
      `bars_override: Some(path_bars)`, `seed: SEED` (the CONSTANT survey `[u8;32]`), keep the **`RunReport`**
      (do NOT `.map(|r| r.kpis)`), map `report.equity_series` → `Vec<Decimal>` (`.amount()` per `Money<Usdt>`),
      and build `PathMetrics` via `compute_sharpe_hourly`/`compute_sortino_hourly`/`compute_calmar`/
      `compute_max_drawdown_f64`/`compute_total_return` + `report.kpis.final_equity/initial_equity.amount()`.
      _Acceptance: harness compiles; `cargo clippy -p backtest --tests -- -D warnings` clean; no new generator/engine/stats. (R-OG.1–3, D-OG.0/D-OG.1)_
- [ ] **T-OG.6** — Add `run_ensemble(root, sym, (start_ms,end_ms), strat, ensemble_seed) -> Option<DistributionSummary>`:
      ...
      the matrix: {sma, macd, rsi, bbands} × {AVAX·2024, DOT·2024} = 8 ensembles + AVAX·2023 SMA contrast (R-OG.5).
      Use a DISTINCT `ensemble_seed` per (strategy×cell). All equity `Decimal`/`Money<Usdt>` until the `compute_*` boundary (R-OG.6).
      _Acceptance: 9 ensembles each return a populated `DistributionSummary` on a corpus-present box; `ScenarioConfig.seed` is the same constant for every path (only `path_seed_j` varies). (R-OG.1/3/6/7, D-OG.3)_
- [ ] **T-OG.7** — Score each `DistributionSummary` against the frozen § 0 bands AS-IS:
      ...
      `prob_loss`, `prob_sharpe_gt_0`, `max_dd_tail_{p50,p95}`, verdict. Do NOT re-derive/soften bands (R-OG.4).
      _Acceptance: each of the 9 ensembles prints a full § 0 summary + composite verdict; thresholds match the § 0 note exactly. (R-OG.4, AC-OG.1)_
- [ ] **T-OG.8** — `#[tokio::test] #[ignore]`; SKIP-clean (print `SKIP`, return) when
      ...
      the determinism (AC-OG.3) + negative-control (AC-OG.4) checks are the miscalibration tripwire. No `.unwrap()`
      outside `#[cfg(test)]` body (the harness IS a test, so the survey's `#![allow(clippy::unwrap_used,…)]` header is fine).
      _Acceptance: default `cargo test -p backtest` stays green (harness ignored); corpus-absent run SKIPs cleanly; `scripts/spec_lint.py` ≤ 70. (R-OG.8/R-OG.10, AC-OG.2/AC-OG.6, D-OG.6)_

> **Watch recipe (emit when kicking off the N=500 × 9 run, > 2 min):**
...
The N=500 × 9-ensemble bootstrap run is a >2 min job. Background + watch:
