...
_architect fills this after Q-resolution. Eight open questions: Q1
(signal source — load-bearing), Q2 (y-snap method), Q3 (tooltip
implementation), Q6 (marker visual treatment), Q7 (histogram widget
shape), Q9 (`SignalView` type shape) — `[ARCHITECT-DECIDE]`. Q4
(tooltip fields), Q5 (layout), Q8 (viewer parity) —
`[OPERATOR-DECIDE]`._

_If Q1 lands on Option (a), Design also names the new
`strategy_signals` row format (or the new `kind` value on
`strategy_events`), the `enable_signal_log` config-gate default
(`false`), and the migration shape (additive, zero downtime)._
