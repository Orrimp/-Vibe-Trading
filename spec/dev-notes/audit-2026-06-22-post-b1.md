...
DOES exist at
`spec/architecture/adr/0038-vol-forecast-verdict-shape.md` (confirmed). The
...
(`spec/architecture/adr/README.md` frontmatter `updated: 2026-06-22 (ADR-0066
added…)`) and the full ADR text exists at
`spec/architecture/adr/0066-benchmark-exempt-from-allfragile.md`. CHANGELOG.md
...
- **Dead-link FAIL (the headline lint violation)** — **RESOLVED in `691e2aa`**,
  ...
  target exists at `spec/architecture/adr/`, the report is byte-immutable). Fixed
