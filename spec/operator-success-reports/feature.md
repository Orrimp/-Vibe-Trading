...
date-stamped markdown file under `spec/operator-success-reports/reports/` is the right
...
- **R1.3** Optional `--output <path>` (default
  `spec/operator-success-reports/reports/<period>-<stamp>.md`, where `<stamp>` is a
  UTC `YYYYMMDD-HHMMSS` slug matching the existing
  `spec/<feature>/reports/backtest-<stamp>-…` convention).
...
- **R3.3** CSV export of the full equity curve under
  `spec/operator-success-reports/reports/artifacts/<run-id>/equity-<window>.csv` —
...
- **R12.1** Three trigger paths:
  ...
  - **(c) On kill-switch trip:** the agent invokes the binary
    ...
    under `spec/operator-success-reports/reports/incident-<halt_event_ts>.md`.
...
**Both scenarios produce reports under `spec/operator-success-reports/reports/`
...
| Q3 | Atomic-write pattern                   | **Tempfile + `rename`** — write to `<output>.tmp.<pid>`, fsync, then `std::fs::rename`. macOS / ext4 / APFS guarantee rename atomicity within the same filesystem; the report path always lives under `spec/operator-success-reports/reports/` (same FS). Same pattern v0 backtest binary uses. |
...
All CSVs live under
`spec/operator-success-reports/reports/artifacts/<run-id>/` and are written atomically
...
`spec/operator-success-reports/reports/incident-<halt_event_ts>.md`. The spawn is
...
pub fn trip(&self, reason: HaltReason) {
    ...
    let _ = std::process::Command::new("cargo")
        .args(["run", "--bin", "report", "--",
               ...
               "--output", &format!("spec/operator-success-reports/reports/incident-{halt_ts_rfc3339}.md")])
...
| **R-5** Atomic-rename fails across filesystems on macOS                                                    | low      | The reports always write to `spec/operator-success-reports/reports/<...>` which lives in the same workspace filesystem as the tempfile. Atomic. Test: `tests/atomic_write.rs` runs against `tempfile::tempdir` (same FS).                                                                                  |
...
`spec/operator-success-reports/tasks.md` for the full task list with
acceptance criteria.
...
- **Anchor capture + regression-gate extension.** Two new entries
  ...
  `success-*-<scenario>.md` under `spec/operator-success-reports/reports/` (the
  existing 9-anchor flow is unchanged byte-for-byte for backtest
  scenarios).  Tests publish a stable `success-fixed-<scenario>.md`
  copy under `spec/operator-success-reports/reports/` so the gate can hash against
  a real on-disk file.
...
- **V3 Both report scenarios run end-to-end.**
  - `report-sample-7d` produces
    `spec/operator-success-reports/reports/report-sample-7d-<stamp>.md` and the
  ...
  - `report-sample-90d` produces
    `spec/operator-success-reports/reports/report-sample-90d-<stamp>.md` plus the
...
- 2026-05-01 (architect): Design section appended. Resolved Q1–Q9
  ...
  `spec/operator-success-reports/tasks.md` (T801–T817 plus
...
- 2026-05-01 (tester): FINAL gate PASS — feature shipped. All 10
  ...
  `spec/archive/test-2026-05-01-1828-operator-success-reports-final.md (archived; see spec/archive/README.md)`.
