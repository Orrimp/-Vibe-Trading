...
- Success-report bodies (verified by grep over
  `spec/operator-success-reports/reports/success-*.md`: zero hits) derive from
...
- `spec/operator-success-reports/reports/success-*.md`: zero matches for the same.
...
| `grep "assets:position\|assets:cash" spec/*/reports/backtest-*.md spec/operator-success-reports/reports/success-*.md` | ZERO hits across all committed report bodies. Anchor risk by construction is zero (Q7). |
...
- `grep "assets:position\|assets:cash" spec/*/reports/backtest-*.md
  spec/operator-success-reports/reports/success-*.md` → ZERO hits.
...
- 2026-05-03 (tester): final gate PASS. T_FINAL_PER_SYMBOL ticked
  in `spec/per-symbol-position-accounts/tasks.md`. All eight V-items
  ...
  report at `spec/archive/test-2026-05-03-0803-per-symbol-position-accounts-final.md (archived; see spec/archive/README.md)`.
  `VERDICT → PASS`.
