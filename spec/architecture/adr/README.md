...
status: in-progress
owner: architect
updated: 2026-06-02 (ADR-0051 § D6.7 amendment — time-series-momentum-robustness: the FIRST non-cross-sectional family (per-asset absolute momentum, long/flat). A 2ND SELECTOR (select_above_threshold) varied at the CONFIG level via a SelectionMode { CrossSectionalTopK (default), TimeSeriesLongFlat } serde-default enum + an entry_threshold field (default ZERO) — the 3rd vary-at-config-not-seed instance, STRICTLY simpler than carry: NO new data source, NO co-resampled series, NO new RNG. The variable-cardinality long/flat (0..N) is PURE on_bar signal emission — run_path/PaperEngine/bootstrap byte-untouched (the engine already sizes 0..N Buys at a fixed fraction + goes to cash on 0 Buys); sizing LOCKED to the existing fixed-fraction (NOT a 1/N rescale). Score = raw Σ log-ret over L (NO vol-norm). 89 anchors byte-identical by construction; separate TS struct rejected (the D6.5.2 trap); +1/+2 TS θ-surface anchors = 90 (2023) / 91 (2024); LOCKED 6-cell grid (lookback {24,168,720} × entry_threshold {0.00,0.02}). prior: 2026-05-31 ADR-0051 § D6.6 REAL-MECHANISM amendment — carry-strategy: a 2ND time series (funding) co-resampled under the SAME shared idx_seq as the returns, ZERO new RNG draws, +1 carry anchor = 88; 2026-05-31 ADR-0051 § D6.5 cross-ref amendment — cross-sectional mean-reversion 2nd strategy family, +1 MR anchor = 87; 2026-05-30 ADR-0045 § D6.3 + § D7.1b engine-drift)
---
