...
status: in-progress
owner: architect
updated: 2026-05-29 (ADR-0050 added: iced-tokio runtime-context contract and cooperative cancellation primitives — codified on 3rd recurrence of Bug #64 D.1.1 per twice-bitten threshold; ADR-0048 § Changelog amended for Bug #64 attempt-3 new e2e tests extending Surface 1 contract)
---
...
| 0050  | iced-tokio runtime-context contract and cooperative cancellation primitives — D1: rt_handle.enter() invariant before any tokio reactor API in iced::Task::perform closures; D2: tokio_util::sync::CancellationToken as canonical cancellation primitive (replaces std::sync::mpsc::sync_channel(0) disconnect-idiom); D3: timer-fired-in-bounded-window test contract (≥ 3 events in 4× interval window); codified on 3rd recurrence of Bug #64 D.1.1 per twice-bitten threshold (instances: P1-fix 2026-05-23, attempt-2 2026-05-29); reference impls: ServerTimeRecipe (live.rs), RunCancelReceiver (cancel.rs), spawn_lab_run preload loop (runner.rs:741-850); e2e tests: lab_runner_ticker_e2e.rs + lab_runner_cancel_e2e.rs | accepted | 2026-05-29 |
