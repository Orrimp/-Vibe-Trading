...
status: in-progress
owner: architect
updated: 2026-05-30 (ADR-0051 added — Monte-Carlo robustness determinism + distribution-report anchoring contract, C1 path-gen + C2 harness; D1-D5 accepted)
---
...
| 0051  | Monte-Carlo robustness — sub-seed derivation, distribution-report shape, and anchor determinism (C1 path-gen + C2 robustness harness). D1 master→path sub-seed rule `path_seed_j = master.wrapping_add(j*0x9E37_79B9)` (reuses the per-symbol idiom on the path axis; bound to index not completion order; ensemble seed orthogonal to fill-tie-break seed which is held constant across paths — resolves direction-note § 8 open-Q2). D2 frozen reduction: index-order sequential mean/two-pass-std + `f64::total_cmp` sort + type-7 linear percentile + NaN-absent assertion (the f64 boundary). D3 distribution-report FM/body split + fixed-precision formatting (`{:.6}` ratios / `{:.2}%` drawdowns) + fixed metric order; every distribution input in the hashed body. D4 anchor unit = ONE summary report under new namespace `mc-robustness-2026-06` (rejects N per-path). D5 determinism scope = Apple-Silicon canonical box, inheriting ADR-0043 f64-conversion-boundary verbatim (cross-platform NOT contracted). Retired-line guard: GARCH/regime as generators-not-alpha; v0.1.0 ships only block bootstrap. | accepted | 2026-05-30 |
