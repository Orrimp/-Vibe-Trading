...
- [x] **T1 (dev)** — Promote the sweep's composed-TOML generators to `pub`.
...
- [x] **T2 (dev)** — Round-trip pin test for the generators (extends ADR-0069
...
- [x] **T3 (dev)** — Add `ForwardParamOverride` (agent-owned closed enum:
...
- [x] **T4 (dev)** — `build_registry_for` honors the override.
...
- [x] **T5 (dev)** — `build_forward_plan_from_registry` honors the SAME
...
- [x] **T6 (dev)** — Divergence + fidelity + plan-reflects-tuned tests
...
- [x] **T7 (ui)** — Add `PromoteParams` (UI-side closed enum mirroring the four
...
- [x] **T8 (ui)** — Add `Message::PromoteSweptConfig(PromoteParams)` to
...
- [x] **T9 (ui)** — Wire `use_config_cell` (`crates/ui/src/screens/tune.rs:971-1019`):
...
- [x] **T10 (dev)** — Binary launch dispatch. In `crates/ui/src/bin/cockpit_live.rs`
...
- [x] **T11 (ui)** — Framing copy in `crate::strings` (zero literals):
...
- [x] **T12 (ui)** — Render-pixel proofs (extend the Tune + forward-plan render
