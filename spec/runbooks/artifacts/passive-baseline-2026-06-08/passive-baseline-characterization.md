...
The runbook (`spec/runbooks/passive-baseline.md`) proposes "monthly rebalance to equal weights"
...
- **No new anchored reports written:** All artifacts are at `spec/runbooks/artifacts/passive-baseline-2026-06-08/`
...
- **Stray report check:** `git status --porcelain` shows only `crates/backtest/Cargo.toml`, `crates/backtest/examples/`, `spec/runbooks/artifacts/`, `spec/runbooks/passive-baseline.md`, and `data/yahoo/REVISION.toml` (pre-existing). Zero files in any `spec/*/reports/` anchored dir.
