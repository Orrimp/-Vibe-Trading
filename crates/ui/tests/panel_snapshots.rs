...
// ── v3-regime-classifier Wave D — regime_tag_column cell snapshots (ADR-0049 § D3) ──

mod regime_tag_column {
    use insta::assert_snapshot;
    use ui::screens::trail::{regime_tag_cell, regime_tag_column_header};
    use ui::theme::ThemeMode;

    /// Smoke-render the widget so a panic in `regime_tag_cell` trips the test.
    fn render_smoke(regime: Option<&str>) {
        let _element: iced::Element<'_, ()> = regime_tag_cell(regime, ThemeMode::Dark);
    }

    /// Pure-text summary for the regime_tag_cell snapshot.
    ///
    /// Captures:
    /// - widget identity (so a rename is caught)
    /// - column_header constant (token name drift)
    /// - the rendered label text
    /// - the Lumen token name for the text color
    fn cell_summary(regime: Option<&str>) -> String {
        use ui::strings::{
            LAB_REGIME_CALM, LAB_REGIME_VOLATILE, TRAIL_COL_REGIME, TRAIL_NO_REGIME_TAG,
        };
        use ui::theme::{ThemeMode, color as t};

        let mode = ThemeMode::Dark;

        let (expected_label, expected_color_name) = match regime {
            Some("bull") => ("bull", "fg"),
            Some("bear") => ("bear", "neg"),
            Some(s) if s == LAB_REGIME_VOLATILE => (LAB_REGIME_VOLATILE, "warn"),
            Some(s) if s == LAB_REGIME_CALM => (LAB_REGIME_CALM, "fg_muted"),
            Some(_) => ("unknown", "fg_dim"),
            None => (TRAIL_NO_REGIME_TAG, "fg_dim"),
        };

        // Verify color tokens match the expected names via the shared color_name helper.
        let expected_color = match expected_color_name {
            "fg" => t::FG_1.current(mode),
            "neg" => t::DOWN_500.current(mode),
            "warn" => t::WARN_500.current(mode),
            "fg_muted" => t::FG_3.current(mode),
            "fg_dim" => t::FG_4.current(mode),
            other => panic!("unexpected color name: {other}"),
        };

        let mut out = String::new();
        out.push_str("widget: regime_tag_cell\n");
        out.push_str(&format!("column_header: {TRAIL_COL_REGIME}\n"));
        out.push_str(&format!("input_regime: {}\n", regime.unwrap_or("None")));
        out.push_str(&format!("label: {expected_label}\n"));
        out.push_str(&format!("color_token: {expected_color_name}\n"));
        // Verify color round-trip through the theme (zero-new-design-tokens gate R-NR.4).
        out.push_str(&format!(
            "color_is_existing_lumen_token: {}\n",
            expected_color != iced::Color::BLACK
        ));
        out
    }

    /// T-D-D2 snapshot 1 — Bull regime cell (FG_1 body text).
    #[test]
    #[allow(non_snake_case)]
    fn __regime_tag_column__bull() {
        render_smoke(Some("bull"));
        assert_snapshot!("__regime_tag_column__bull", cell_summary(Some("bull")));
    }

    /// T-D-D2 snapshot 2 — Bear regime cell (DOWN_500 semantic red).
    #[test]
    #[allow(non_snake_case)]
    fn __regime_tag_column__bear() {
        render_smoke(Some("bear"));
        assert_snapshot!("__regime_tag_column__bear", cell_summary(Some("bear")));
    }

    /// T-D-D2 snapshot 3 — Volatile regime cell (WARN_500 semantic amber).
    #[test]
    #[allow(non_snake_case)]
    fn __regime_tag_column__volatile() {
        render_smoke(Some("volatile"));
        assert_snapshot!(
            "__regime_tag_column__volatile",
            cell_summary(Some("volatile"))
        );
    }

    /// T-D-D2 snapshot 4 — Calm regime cell (FG_3 muted).
    #[test]
    #[allow(non_snake_case)]
    fn __regime_tag_column__calm() {
        render_smoke(Some("calm"));
        assert_snapshot!("__regime_tag_column__calm", cell_summary(Some("calm")));
    }

    /// T-D-D2 snapshot 5 — None (no regime tag) cell (FG_4 dim placeholder).
    #[test]
    #[allow(non_snake_case)]
    fn __regime_tag_column__none() {
        render_smoke(None);
        assert_snapshot!("__regime_tag_column__none", cell_summary(None));
    }

    /// T-D-D2 — column header constant gate.
    /// The header label must match the TRAIL_COL_REGIME string constant.
    #[test]
    fn regime_tag_column_header_matches_constant() {
        use ui::strings::TRAIL_COL_REGIME;
        assert_eq!(regime_tag_column_header(), TRAIL_COL_REGIME);
    }
}
