...
// NOTE (ui-drop-iced-aw, 2026-05-16): the iced_aw::Badge Catalog
// adapter (cockpit_badge_style + cockpit_badge_style_fn, ~130 LOC)
// was removed when iced_aw was dropped. Status pills now render via
// native iced Container+Text in `widgets::strategies::status_badge_cell`
// using the same Lumen palette pairs (UP_50/UP_500, ACCENT_SOFT/FG_3,
// DOWN_50/DOWN_500). `BadgeIntent` enum above is retained because
// strategies.rs still maps StrategyStatus → BadgeIntent for colour
// routing.
...
mod tests {
    ...
    // NOTE: badge Catalog tests removed alongside cockpit_badge_style
    // when iced_aw was dropped (ui-drop-iced-aw, 2026-05-16). Native
    // status_badge_cell in widgets/strategies.rs is covered indirectly
    // by the visual snapshot suite.
}
