...
//!   reports under `evidence/v1/backtest-real-binance-data/reports/` are NEVER
//!   touched.
...
struct Args {
    ...
    /// Defaults: tcn → evidence/v1/v25a-patchtst-overlay/reports/,
    ///           vol-target-bs1 → evidence/v1/v3-volatility-forecaster/reports/,
    ///           regime-dispatcher-bs1 → evidence/v1/v3-regime-classifier/reports/.
    #[arg(long)]
...
fn main() -> Result<()> {
    ...
    let out_dir: PathBuf = args.out_dir.clone().unwrap_or_else(|| match args.scenario {
        ScenarioFamily::VolTarget => PathBuf::from("evidence/v1/v3-volatility-forecaster/reports/"),
        ScenarioFamily::VolTargetRebaseline => {
            PathBuf::from("evidence/v1/v3-volatility-forecaster-rebaseline/reports/")
        }
        ScenarioFamily::RegimeDispatcher => {
            PathBuf::from("evidence/v1/v3-regime-classifier/reports/")
        }
        ScenarioFamily::Tcn => PathBuf::from("evidence/v1/v25a-patchtst-overlay/reports/"),
    });
    ...
    let source_reports: Vec<String> = rerun::SCENARIOS
        .iter()
        .map(|s| format!("evidence/v1/backtest-real-binance-data/reports/backtest-…-{s}.md"))
        .collect();
