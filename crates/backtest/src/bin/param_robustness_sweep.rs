...
//! - g=0: baseline θ* (orientation probe — disclosed DIRECTION-match vs the C2
//!   anchor only, checked by manual eyeball; C2 ran N=500 vs this sweep's N=200,
//!   so the numbers do NOT reproduce and no automated compare exists — review 1-15 M3)
...
// ── θ-grid + sweep axes + renderer — relocated to `backtest::sweep_harness` ────
//
// Review 1-15 (H2/H3/M1, the 1-14 `mc_harness` precedent): the θ-grid
// definitions (`ThetaCell` + the LOCKED grids), the sweep-axis enums, the
// per-cell config injection (`cell_config`), the grid-definition formatters,
// the θ-surface renderer (`render_surface_report`), the family-verdict single
// source (`family_any_non_fragile`/`family_verdict_line`), and the
// scenario-identity builder (`build_scenario_name`) were extracted VERBATIM
// into the `backtest` library so the day-1 e2e gates
// (`tests/param_sweep_e2e.rs`) exercise the REAL production chain instead of
// local re-implementations (the #66 vacuous-test class). This bin re-imports
...
/// mark-to-market equity (no rebalancing). The path is entirely FEE-FREE by
/// design, INCLUDING the bar-0 entry — no entry fee is charged anywhere
/// (review 1-15 L2 truthfix: the old comment claimed "no fees after bar 0",
/// implying a bar-0 fee that does not exist). Whether the passive control
/// SHOULD pay the entry fee the θ-cells pay is a behavioral question owned by
/// story 1-25 (anchor #86 re-derivation program).
#[must_use]
...
fn load_source_bars(
    ...
    match args.generator {
        GeneratorKind::GbmSmoke => {
            // Review 1-15 L3: the gbm lane generates its bars PER-PATH inside
            // `run_one_path_with_config` (and the BUYHOLD closure) — a gbm
            // "source" series is never consumed by any code path (every
            // consumer of `real_bars_by_symbol` is gated on
            // `GeneratorKind::BlockBootstrapReal`). The old code burned
            // 87,600 unused bars here, seeded from a literal `0xC0FFEE` base
...
/// N × G parallel path tasks via `&` references (re-scoped default: N=200 ×
/// 6 cells; the pre-re-scope design was N=500 × 14). This avoids the O(N×G)
/// clone of the ~87,600-bar full-year source that the original per-task
/// construction caused.
...
/// across all N tasks for a given cell (re-scoped default N=200) via `&`
/// references. Since rayon
        ...
        GeneratorKind::GbmSmoke => {
            let bars_by_symbol: Vec<Vec<trading_core::Bar>> = universe
                ...
                .map(|(sym_i, (sym, start_price))| {
                    let sym_seed = backtest::mc_harness::derive_gbm_sym_seed(path_seed_j, sym_i);
...
fn main() -> Result<()> {
    ...
    // Review 1-15 L4: the ratified floor for decision-grade surfaces is N ≥ 200
    // (the 2026-05-30 orchestrator re-scope: 6 cells × N=200). Small-N runs stay
    // LEGAL — N is a hashed body field and tests/probes legitimately run tiny N —
    // but they must be loudly flagged as non-decision-grade. Warn, never bail.
    if args.paths < 200 {
        tracing::warn!(
            n_paths = args.paths,
            "--paths below the ratified N=200 floor (2026-05-30 re-scope): \
             this surface is NOT decision-grade; verdicts at small N are indicative only"
        );
        eprintln!(
            "WARNING: --paths {} is below the ratified N=200 floor (2026-05-30 re-scope). \
             The emitted surface is NOT decision-grade; verdicts at small N are \
             indicative only (N is a hashed body field, so the run is still recorded honestly).",
            args.paths
        );
    }
    ...
    // (re-scoped default: N=200 × 6 cells; was N=500 × 14 pre-re-scope).
    ...
    let bh_results: Vec<(usize, backtest::stats::PathMetrics)> = sweep_pool.install(|| {
        bh_path_indices
            .into_par_iter()
            .map(
                |j| -> anyhow::Result<(usize, backtest::stats::PathMetrics)> {
                    let path_seed_j = derive_path_seed(ensemble_seed, j);
                    let generated_path = match args.generator {
                        GeneratorKind::BlockBootstrapReal => {
                            use data::MonteCarloPathGen as _;
                            let path_gen = block_path_gen_opt.as_ref().context(
                                "block_path_gen_opt must be Some for BlockBootstrapReal",
                            )?;
                            path_gen
                                .generate(&universe, bar_count, path_seed_j)
                                .with_context(|| {
                                    format!("BUYHOLD control: path j={j} generation failed")
                                })?
                        }
                        GeneratorKind::GbmSmoke => {
                            let bars_by_symbol: Vec<Vec<trading_core::Bar>> = universe
                                ...
                                .enumerate()
                                .map(|(sym_i, (sym, start_price))| {
                                    let sym_seed = backtest::mc_harness::derive_gbm_sym_seed(
                                        path_seed_j,
                                        sym_i,
                                    );
                                    backtest::scenarios::momentum::synthetic_bars_hourly(
