...
**Process note — the part that matters**: the 1-18 developer *observed this exact behaviour*, described it accurately in a test comment ("the risk guard silently rejects the SELL … leaving the physical position open forever"), and shipped a **gentler fixture** (`build_1h_up_down_bars_moderate`, +0.1%/bar instead of +1%/bar) so the cap would not trip — instead of filing it. The falsifier was tuned green around a live correctness bug.
...
**Status**: OPEN — owned by story 1-25; the durable claim is already qualified (`evidence/v1/horizon-retest-robustness/reports/ERRATA-2026-08-04.md`). Disclosure entry, 2026-08-04.
