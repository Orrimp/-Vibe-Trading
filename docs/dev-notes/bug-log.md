...
**Scope check before anyone panics.** This does not touch the ship-passive thesis: the advisor gate (`bakeoff/bootstrap.rs`) resamples log-returns and the crowned arm across the corpus is the passive benchmark, not a short arm. The harm is that a short arm's ranking is flattered relative to its long siblings — a comparability defect, not a wrong headline. **Anchor impact is unmeasured**: the research lanes that DO write anchored bodies (`montecarlo.rs`, `threshold_sweep.rs`) have their own order paths, so whether any anchored surface runs short legs through `short_exec` needs checking before this is fixed — do that first, because fixing it would move those numbers.
...
**Fix**: add `features = ["yahoo"]` to the `backtest` dependency wherever the product builds it (or make it a default), **and** add a test that fails when the module is absent — the current `#[cfg]` makes absence silent, which is why this survived. Then fix the leg-alignment defect above. Until both land, the arm must be **dropped from the ranked field** (story 3-16 already owns that gap — `bakeoff/mod.rs` has a drop-to-ABSENCE guard for the DVOL sibling and none for macro).
