...
│   │   ├── architecture/decisions/       # was _bmad-output/planning-artifacts/architecture/decisions/ (86 ADRs + README registry)
...
| `adr_registry_check.py` | path regex `_bmad-output/planning-artifacts/architecture/decisions/*.md` ×3, README path | Repoint to `planning-artifacts/architecture/decisions/`. |
...
| `operator_ledger_check.py` | `_DEVNOTE_RE = docs/dev-notes/…`, default ledger `docs/dev-notes/operator-side-pending-ledger.md` | Repoint regex + default to `docs/dev-notes/`. |
...
- `crates/ui/src/strings.rs` — `KILL_RUNBOOK_LINK_PATH = "docs/runbooks/kill-switch.md"` → `docs/runbooks/kill-switch.md`; `MODELS_EMPTY_STATE` doc ref.
- `crates/ui/src/baseline/loader.rs` + `crates/ui/tests/baseline_error_state.rs` — `docs/runbooks/artifacts/passive-baseline-2026-06-08` → `docs/runbooks/artifacts/…`.
