...
| DEFERRED-KNOWN | ADRs 0045–0049 absent from `_bmad-output/planning-artifacts/architecture/decisions/README.md` registry table (5-entry gap) | `_bmad-output/planning-artifacts/architecture/decisions/README.md` | Carry-over from prior audits; ADRs 0050+0051 ARE registered and correct |
...
- `REQ-VISUAL-FAIL-HTML-REPORTER-001` arch field: `docs/dev-notes/archive/2026-Q2/ui-testability-deep-dive-2026-05-15.md` — archived path not resolvable.
...
- `_bmad-output/planning-artifacts/architecture/decisions/0027-kronos-onnx-tract-integration.md` — 5 links to `../../v25-kronos-forecast-overlay/feature.md` (slug no longer exists; archived).
...
- `_bmad-output/planning-artifacts/architecture/decisions/0045-v5-canonical-config-and-noop-baseline-namespace.md` — link to `../../.claude/skills/spec-update/SKILL.md` (relative path from ADR subdir resolves incorrectly).
...
Grepped all `spec/` folders (excluding `spec/archive/` and `docs/dev-notes/`) for `TODO`, `FIXME`, `TBD`, `???`, `XXX`. No new decay-heavy folders introduced by today's churn.
...
- `_bmad-output/planning-artifacts/architecture/decisions/0051-monte-carlo-determinism-and-distribution-report-anchoring.md` § D6.6 covers the determinism and anchoring contract for carry, confirming Q-CARRY-3 (the crux) is tractable and zero new RNG draws are needed.
