---
name: rust-test
description: Run the full Rust test suite and render a structured markdown report. Use after a build succeeds, on every PR, and whenever the tester agent is invoked. Produces a report at evidence/<slug>/reports/test-<timestamp>-<slug>.md using the template in this skill.
---
...
6. If backtests are in scope for this run, invoke the `backtest` skill and embed
   ...
   `scripts/prune_backtest_duplicates.sh` so `evidence/*/reports/` keeps exactly
